...
import bountyFoxesJson from "../assets/500BountyFoxes.json";
import bountyFoxesLogo from "../assets/bounty_foxes_logo.png";
...
type BountyFoxesProps = {

}

export const BountyFoxes = ({ }: BountyFoxesProps) => {
  //loading for twins
  ...
  useEffect(() => {
    ...
    if (addresses?.ordAddress) {
      ...
      useEffect(() => {
        ...
        setOrdinalsString(JSON.stringify(bountyFoxesJson));
  ...
  const handleNumOwned = async (num: number, outpoints: string[]) => {
    ...
    // // console.log('Received outpoints in BountyFoxes:', outpoints);
  ...
  useEffect(() => {
    ...
    const fetchOwnedFoxes = async () => {
      try {
        const response = await fetch(`https://ordinals.gorillapool.io/api/txos/address/${myordaddress}/unspent?limit=300000`);
        if (!response.ok) {
        ...
        // Get all outpoints from bountyFoxesJson
        const bountyFoxOutpoints = bountyFoxesJson.map(fox => fox.origin.outpoint);
        
        // Filter owned foxes to only include those in bountyFoxesJson
        const ownedFoxes = data.filter(fox => 
          bountyFoxOutpoints.includes(fox.origin.outpoint)
        );
...
<br />
              ...
              <>
                ...
                <ThreeCircles color="white" height="50"
                  width="50" />
                                ...
                                {!setgettingfox && (
              <>
        <p className="Heading"><span className="Demo"><b>Bounty Foxes Group</b></span><br />Sort 500 Bounty Foxes<br /><span style={{ color: '#808080', fontSize: '0.9em' }}>"Bounty foxes from tag"</span><br /><span className="GrayGroupLinks"><a target="blank" href="https://ordinals.gorillapool.io/content/7a85e1d3de33e43c3706ae21fe93e41a88ae08b58b7b6f124f6ccc4827d26cc3_0" style={{ color: '#808080' }}><u>Group</u></a></span></p>
        ...
        <div className="WhiteGroupLinks" style={{ textAlign: 'center', width: '100%', margin: '5px auto' }}>
          ...
          )} <span style={{ color: '#808080' }}>/</span> <span style={{ color: '#808080' }}>{bountyFoxesJson.length} Bounty Foxes</span>
